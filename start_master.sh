...
function start-master() {
  ...
  echo "Loading K8s images from tar.gz"
  images/load_all_images.sh &
  ...
  sed -i 's/192.168.0.0\/16/'${pod_cidr_slash}'/' images/calico.yaml
  kubectl --kubeconfig="${config}" apply -f images/calico.yaml
  ...
  # Start dasboard and proxy
  kubectl apply -f images/kubernetes-dashboard.yaml
  ...
  sleep 30
  while :; do clear && kubectl get pods --all-namespaces -o wide; sleep 5; done
